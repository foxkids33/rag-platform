...
документа. До применения этих фильтров детерминированный query-aware selector
объединяет retrieval quality с покрытием значимых терминов вопроса. Точные числа,
версии, даты и аббревиатуры получают больший вес; доменные названия и aliases в
коде не зашиты. Для результатов cross-encoder влияние lexical-сигнала ограничено,
чтобы не перезаписывать сильный reranker. Вес задаётся через
`RAG_QUERY_SELECTION_WEIGHT` и может быть установлен в `0` для чистого A/B.
В источниках доступны `query_relevance_score` и итоговый `selection_score`.
При недостаточных доказательствах API возвращает детерминированный
...
Если retrieval-контекст выглядит релевантным по score, модель начинает вывод с
внутреннего маркера `[[ANSWER]]` или `[[ABSTAIN]]`. API удаляет управляющие
маркеры из JSON и SSE независимо от границ токенов и их позиции. Для отказа API
выставляет `abstained=true` и записывает причину в `abstention_reason`. Значение
`insufficient_retrieval_evidence` означает отказ
до вызова LLM, а `model_reported_insufficient_context` — отказ после проверки
контекста моделью. Для корректного отказа отсутствие ссылок не считается
ошибкой citation audit; контекстный отказ может сохранить короткое объяснение и
валидную ссылку. SSE-событие `done` содержит итоговый статус.
Если источники подтверждают только часть запроса, модель отвечает по этой части
со ссылками и явно обозначает пробел вместо полного отказа. Для вопроса, целиком
требующего отсутствующее точное число, дату или версию, сохраняется полный отказ.
API хранит все переданные модели фрагменты для диагностики, а интерфейс показывает
в блоке «Использованные источники» только индексы из `cited_source_indices`, в том
числе для контекстного безопасного отказа.
